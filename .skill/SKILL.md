...
| **要新增文章 / 改自動發文** | `references/auto-publish.md` — 智財知識專欄自動發文（GitHub Actions + FLUX 生圖）|
| **改/建智財新聞自動更新** | `references/auto-news.md` — 智財新聞半自動系統（爬取+改寫+來源標注）|
| **排程/為何沒自動發文** | `references/scheduling.md` — 排程靠 Ubuntu cron 觸發（非 GitHub schedule）|
